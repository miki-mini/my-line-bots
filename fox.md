...
```mermaid
graph TD
    User((User)) -->|YouTube URL| LINE[LINE Messaging API]
    LINE -->|Webhook| CloudRun[Google Cloud Run / FastAPI]
    CloudRun -->|Get Transcript| YouTube[YouTube Data]
    CloudRun -->|Search Query| Google[Google Search]
    CloudRun -->|Context & Summary| Gemini[Gemini API]
