...
def register_owl_handler(app, auth_dependency=None):
    ...
    Parameters:
        app: FastAPIアプリケーション
        auth_dependency: Basic認証用のDependency (Webアプリ用)
    """
    ...
    # ==========================================
    # 🌍 1. Legacy Endpoints (No Auth - for GAS)
    # ==========================================
    ...
    # ==========================================
    # 🔐 2. Secured Endpoints (Basic Auth - for Web App)
    # ==========================================
    # auth_dependencyが提供されている場合のみ登録
    if auth_dependency:
        @app.post("/api/owl/analyze_image", dependencies=[auth_dependency])
        async def analyze_image_secure(image_file: UploadFile = File(...)):
            return await _process_image_analysis(image_file)

        @app.post("/api/owl/record_weight", dependencies=[auth_dependency])
        async def record_weight_secure(req: WeightRequest):
            return await _process_weight_recording(req)

        @app.get("/api/owl/graph/weight", dependencies=[auth_dependency])
        async def get_weight_graph_secure():
            return await _generate_weight_graph()

        @app.get("/api/owl/graph/calories", dependencies=[auth_dependency])
        async def get_calories_graph_secure():
            return await _generate_calories_graph()

    print("🦉 フクロウハンドラー登録完了（ハイブリッド構成: Legacy + Secure）")